  ...
  try {
    ...
    if (data) {
      return {
        ...
        otherComponents: data.other_components || [],
      };
    }

    return { checklist: {}, prices: {}, partNames: {}, totalBudget: 100000, currency: "INR", otherComponents: [] };
  } catch (error) {
...
export const updateUserCurrency = async (newCurrency) => {
  try {
    const {
      data: { user },
    } = await supabase.auth.getUser();
